"""SvelteKit filesystem routes + components → dr_route / dr_component.

This repo is a SvelteKit app (adapter-static). Routes are the filesystem: files
under a `routes/` dir named `+page.svelte` / `+page.*` / `+server.*` /
`+layout.svelte` map to a URL derived from the directory path (`[param]` →
`:param`, `[...rest]` → `*rest`, `(group)` dropped). Every other `*.svelte` file
is a component. Static best-effort — no dynamic/computed routes in this repo.
"""
import re
